...
#----------------
class Network_Baseline_Model (object): #this is the model presented in the thesis (chapter 6 as the competitive model number 2)
    def __init__ (self,landa,p,q,mu,adjacency):
        number = 0
        self.network_all_node = []
        self.timestep = 0
        self.number_of_nodes_in_S_state_per_timestep = []
        ...
        self.number_of_nodes_in_R_state_per_timestep = []
    ...
    def number_of_nodes_in_each_state_per_timestep (self) : 
        ...
        self.number_of_nodes_in_R_state_per_timestep.append(R_number)
    ...
    def change (self,landa,p,q,mu,t) : #These parameters corrospond to their corrospondants in the origin model respectively as Beta1, Beta2, Beta2, Mu1, Beta1, Mu2, Mu1, Mu2
        import random
        ...
        for node in self.network_all_node :
            ...
            if node.state == "S" :
                ...
                #calculating number of I12 that act as I1
                number_I12_state_neighbor_as_I1 = number_I12_state_neighbor * (p/(p+q))

                #calculating number of I12 that act as I2
                number_I12_state_neighbor_as_I2 = number_I12_state_neighbor * (q/(p+q))
                ...
                if number_I1_state_neighbor + number_I2_state_neighbor + number_I12_state_neighbor != 0: #means at least there is one neighbour who is in I1 or I2 state
                    ...
                    a = (landa*(number_I1_state_neighbor+number_I12_state_neighbor_as_I1))/(1-0.5*landa)
                    b = (landa*(number_I2_state_neighbor+number_I12_state_neighbor_as_I2))/(1-0.5*landa)
                    c = 1-((1-landa)**(number_I1_state_neighbor+number_I12_state_neighbor_as_I1))*((1-landa)**(number_I2_state_neighbor+number_I12_state_neighbor_as_I2))
                ...
                if x <= P_S_TO_I1 :
                    ...
                    node.primary_state = "I2"
                elif x <= (P_S_TO_I1 +  P_S_TO_I2 + P_S_TO_I12)  :
                ...
                for neighbor in node.neighbor :
                    if neighbor.state == "I1" :
                        number_I1_state_neighbor += 1
                    if neighbor.state == "R" :
                        number_R_state_neighbor += 1
                        
##                #computing non_spontaneous_transition_probability
##                non_spontaneous_transition_probability = 1 - GAMMA_SI1 - GAMMA_SI2         
                if number_I1_state_neighbor + number_R_state_neighbor != 0: #means at least there is one neighbour who is in I1 or I2 state
                    # Computing of required parameters : {{{                        
                    #computing F_I1I12 and F_I1R1
                    a = 0
                    b = (mu*(number_I1_state_neighbor + number_R_state_neighbor))/(1-0.5*mu)
                    c = 1-((1-mu)**(number_I1_state_neighbor + number_R_state_neighbor))
                    d = a + b #we used float to stop machine from rounding small numbers to zero and cause us to have division by zero
                    if d == 0 :
                        d = 1.0e-323 #since numbers less than this, will be set as 0 by python and this causes us to have division by zero, we set this small number for them                
...
##                #computing non_spontaneous_transition_probability
##                non_spontaneous_transition_probability = 1 - GAMMA_SI1 - GAMMA_SI2         
                if number_I2_state_neighbor + number_R_state_neighbor != 0: #means at least there is one neighbour who is in I1 or I2 state
                    # Computing of required parameters : {{{                        
                    #computing F_I1I12 and F_I1R1
                    a = 0
                    b = (mu*(number_I2_state_neighbor+number_R_state_neighbor))/(1-0.5*mu)
                    c = 1-((1-mu)**(number_I2_state_neighbor+number_R_state_neighbor))
                    d = a + b #we used float to stop machine from rounding small numbers to zero and cause us to have division by zero
                    if d == 0 :
                        d = 1.0e-323 #since numbers less than this, will be set as 0 by python and this causes us to have division by zero, we set this small number for them                
                ...
                if number_I1_state_neighbor + number_I2_state_neighbor + number_I12_state_neighbor + number_R_state_neighbor != 0: #means at least there is one neighbour who is in I1 or I2 state
                    # Computing of required parameters : {{{                        
                    #computing F_I1I12 and F_I1R1
                    a = (p*(number_I1_state_neighbor+number_I12_state_neighbor_as_I1))/(1-0.5*p)
                    b = (q*(number_I2_state_neighbor+number_I12_state_neighbor_as_I2))/(1-0.5*q)
                    e = (mu*(number_R_state_neighbor))/(1-0.5*mu)
                    
                    c = 1-((1-p)**(number_I1_state_neighbor+number_I12_state_neighbor_as_I1))*((1-q)**(number_I2_state_neighbor+number_I12_state_neighbor_as_I2))*((1-mu)**(number_R_state_neighbor))
                    d = a + b + e #we used float to stop machine from rounding small numbers to zero and cause us to have division by zero
                    if d == 0 :
                        d = 1.0e-323 #since numbers less than this, will be set as 0 by python and this causes us to have division by zero, we set this small number for them                
                    F_I12I1 = (a*c)/d
                    F_I12I2 = (b*c)/d
                ...
                if x <= P_I12_TO_I1 :
                    node.primary_state = "I1"   
                elif x <= (P_I12_TO_I1 +  P_I12_TO_I2)  :
                    node.primary_state = "I2"
                elif x <= (P_I12_TO_I1 +  P_I12_TO_I2 + P_I12_TO_R)  :
                    node.primary_state = "R"
                else :
                    node.primary_state = "I12"
        ...
        for node in self.network_all_node :
            ...
            elif node.state == "S" and node.primary_state == "I12" :
                node.STOI12(t)

            elif node.state == "I1" and node.primary_state == "R" :
                node.I1TOR(t)

            elif node.state == "I2" and node.primary_state == "R" :
                node.I2TOR(t)

            elif node.state == "I12" and node.primary_state == "R" :
                node.I12TOR(t)

            elif node.state == "I12" and node.primary_state == "I1" :
                node.I12TOI1(t)

            elif node.state == "I12" and node.primary_state == "I2" :
                node.I12TOI2(t)
...
def param_baseline (down_limit,up_limit): #here there is no diference between new_model params and baseline_model params
    landa = infection (down_limit,up_limit)
    p = infection (down_limit,up_limit)
    q = infection (down_limit,up_limit)
    mu = infection (down_limit,up_limit)
...
def main():
    ...
    while j < number_expriment :
        ...
        landa,p,q,mu = param_baseline (down_limit,up_limit)
        NBM = Network_Baseline_Model(landa,p,q,mu,adjecency)
        ...
        param_set_baseline=[landa,p,q,mu]
