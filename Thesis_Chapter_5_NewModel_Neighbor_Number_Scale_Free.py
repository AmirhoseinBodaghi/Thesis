class Node (object) :
    ...
    def get_neighbor (self,all_node,G):
        self.neighbor = []
        for number in list(G.predecessors(self.number)) :
...
class Network_New_Model (object):
    def __init__ (self,BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,G):
        number = 0
        ...
        for node in self.network_all_node :
            node.get_neighbor (self.network_all_node,G)
    ...
    def change (self,BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,t) :
        import random
        ...
        for node in self.network_all_node :
            ...
            if node.state == "S" :
                ...
                P_S_TO_I1 = GAMMA_SI1 + non_spontaneous_transition_probability * G_SI1
                P_S_TO_I2 = GAMMA_SI2 + non_spontaneous_transition_probability * G_SI2
                P_Remain_S = non_spontaneous_transition_probability  * G_S0
...
def graph_generator ():
    import networkx as nx
    G = nx.scale_free_graph(1000,alpha=0.43, beta=0.56, gamma=0.01) ######## here we make a scale free network which is directed :) please notice : list(G.predecessors(n)) gives us the list of nodes that are following of node n, ie they can cause a change in state of node n and in the graph there is a arrow from them to node n, and list(G.neighbors(n)) gives us the list of nodes that are followers of node n, ie node n can cause a change in state of them and in the graph there is a arrow from node n to them
...
def param_new_model (down_limit,up_limit):
    ...
    return BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2
#------------
...
def main():
    ...
    while j < number_expriment :
        ...
        BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2 = param_new_model (down_limit,up_limit)
        NNM = Network_New_Model(BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,G)
        ...
        while t < number_timestep + 1 :
            NNM.change(BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,t)
            NNM.time (t)
            NNM.number_of_nodes_in_each_state_per_timestep ()
            t+=1
        ...
        BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2 = param_baseline (down_limit,up_limit)
        NBM = Network_Baseline_Model(BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,G)
        ...
        while t < number_timestep + 1 :
            NBM.change(BETA_SI1,BETA_SI2,GAMMA_SI1,GAMMA_SI2,t)
            NBM.time (t)
            NBM.number_of_nodes_in_each_state_per_timestep ()
            t+=1
