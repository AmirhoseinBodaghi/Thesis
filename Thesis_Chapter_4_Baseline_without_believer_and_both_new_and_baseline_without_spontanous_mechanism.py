...
def plot_results (x,y,w,q,r,z) : 
    ...
    while h < z + 1: #making the horizental line of plot as the number of timesteps
        f.append(h)
        h+=1
    file_name = "The number of nodes in each state per timestep"
    ax = plt.axes ()
    ax.axes.get_xaxis().set_visible(True)
    ax.axes.get_yaxis().set_visible(True)
    plt.plot(f,x,'r' , f,y,'b' , f,w,'y' , f,q,'c' , f,r,'g') #x for I1 , y for I2 , w for B1 , q for B2 , r for S
    plt.xlabel('Timestep')
    plt.ylabel('Number of Nodes in States')
##    plt.title('New Rumor Model Spreading on Scalefree Network : {red} I1 Nodes & {green} S Nodes & {blue} I2 Nodes & {yellow} B1 Nodes & {cyan} B2 Nodes',fontsize=5)
    plt.grid(True)
##    landa_s = str(landa)
##    beta_s = str(beta)
##    data1 = ('Infection Rate =',landa_s)
##    data2 = ('Recovery Rate =',beta_s) 
##    plt.text(1.3,625,data1,fontsize=8) #by these we put data on the plot
##    plt.text(1.3,575,data2,fontsize=8) #by these we put data on the plot
    path_of_saving = str ("C:\\Users\\novin\\Desktop\\PhD_Programs\\NewModel_Origin\\Main_Code\\Output\\Pics_of_netwrok_in_timesteps_by_matplotlib") #here we make the address of the folder we want pictures to be saved, BUT YOU HAVE TO douplicate each Backslash in the URL address (as you see I did ) , if you dont it wont work and give you error of UNICODESCAPE !
...
def main():
    ...
    while j < number_expriment :
        ...
        lattice = Network(BETA_SI1,BETA_SI2,BETA_B1I1,BETA_B1I2,BETA_B2I2,BETA_B2I1,GAMMA_B1I1,GAMMA_B2I2,GAMMA_SB1,GAMMA_SI1,GAMMA_SB2,GAMMA_SI2,GAMMA_B1S,GAMMA_B2S,T_SI1I1,T_SI2I2,adjecency)
        lattice.initial_infection (g,f) # this function makes an initial infected node NOT randomly! but we choose TWO certain nodes as infected to I1 & as infected to I2 because we want to just study influence of transmission parameters so we do not include other factors 
        t = 0
        lattice.number_of_nodes_in_I1_state_per_timestep = []
        lattice.number_of_nodes_in_I2_state_per_timestep = []
        lattice.number_of_nodes_in_each_state_per_timestep ()
        while t < number_timestep + 1 :
            lattice.change(BETA_SI1,BETA_SI2,BETA_B1I1,BETA_B1I2,BETA_B2I2,BETA_B2I1,GAMMA_B1I1,GAMMA_B2I2,GAMMA_SB1,GAMMA_SI1,GAMMA_SB2,GAMMA_SI2,GAMMA_B1S,GAMMA_B2S,T_SI1I1,T_SI2I2)
            lattice.time (t)
            lattice.number_of_nodes_in_each_state_per_timestep ()
            t+=1
        error_new_model = error_computing (lattice.number_of_nodes_in_I1_state_per_timestep,lattice.number_of_nodes_in_I2_state_per_timestep,dataset_number)
        ...
        while t < number_timestep + 1 :
            ...
            lattice.number_of_nodes_in_each_state_per_timestep ()
            t+=1
        error_baseline = error_computing (lattice.number_of_nodes_in_I1_state_per_timestep,lattice.number_of_nodes_in_I2_state_per_timestep,dataset_number)
